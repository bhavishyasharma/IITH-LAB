...
def is_vertex(A,B,sol,n):
    ...
    for i in range(len(B)):
        if(round(B[i],2)==round(thisx[i],2)):
            tight=tight+1
    return (tight==n)
...
def main():
    ...
    while(not is_vertex(matrixA, vectorB, possible_solution, col) and check_constraint(matrixA, vectorB, possible_solution)):
        ...
        possible_solution = next_vertex(matrixA, vectorB, possible_solution, along, col)
